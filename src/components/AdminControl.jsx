        ...
        <div className="flex justify-between">
          {Object.keys(overValue).map((key, index) => {
            ...
            return (
              <div key={key}>
                <div
                  className={`flex items-center  justify-center sm:w-16 w-9   text-${textColor} sm:h-16 h-9 border ${
                    currentBall == key
                ...
                <span className="text-red-500 300:text-xs  375:text-sm 425:text-lg">{`${
                  parseInt(key) + 1
                } ball`}</span>
                {currentBall === index ? (
                  <img
